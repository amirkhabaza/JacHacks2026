/* Source: /Users/amirkhabaza/JacHacks2026/frontend.jac */
import {__jacJsx, __jacSpawn} from "@jac/runtime";
import { __jacCallFunction } from "@jac/runtime";
import { Nav } from "./components/Nav.js";
import { HeroSection } from "./components/HeroSection.js";
import { InteropSection } from "./components/InteropSection.js";
import { FullstackSection } from "./components/FullstackSection.js";
import { MicroservicesSection } from "./components/MicroservicesSection.js";
import { ByllmSection } from "./components/ByllmSection.js";
import { NativeSection } from "./components/NativeSection.js";
import { WasmShooterSection } from "./components/WasmShooterSection.js";
import { LittleXSection } from "./components/LittleXSection.js";
import { SourceViewer } from "./components/SourceViewer.js";
import { SeamsSection } from "./components/SeamsSection.js";
import { OutroSection } from "./components/OutroSection.js";
import { Router, Routes, Route } from "@jac/runtime";
import { app as LittleX } from "./littlex/frontend.js";
import "./global.css";
import { useState } from "@jac/runtime";
import { useEffect } from "@jac/runtime";
async function guestbook_stats() {
  return await __jacCallFunction("guestbook_stats", {});
}
async function list_source_files() {
  return await __jacCallFunction("list_source_files", {});
}
async function get_source_file(path) {
  return await __jacCallFunction("get_source_file", {"path": path});
}
const _jac = {
  int: {
    bit_length(n) {
...
function App() {
  return __jacJsx(Router, {}, [__jacJsx(Routes, {}, [__jacJsx(Route, {"path": "/littlex", "element": __jacJsx("div", {"className": "h-screen overflow-hidden bg-background text-foreground"}, [__jacJsx(LittleX, {}, [])])}, []), __jacJsx(Route, {"path": "/*", "element": __jacJsx(Showcase, {}, [])}, [])])]);
}
const app = App;
function Showcase() {
  ...
  const [gbSigning, setGbSigning] = useState(false);
  const [graphNonce, setGraphNonce] = useState(0);
  const [walkerSnap, setWalkerSnap] = useState(null);
  ...
  async function loadInitial() {
    let gb = await __jacSpawn("get_guestbook", "", {});
    setGuestbook((_jac.builtin.bool(gb.reports) ? gb.reports[0] : []));
  }
  async function signGuestbook() {
    ...
    setGbSigning(true);
    await __jacSpawn("sign_guestbook", "", {"name": gbName, "message": gbMessage});
    setGbMessage("");
    let gb = await __jacSpawn("get_guestbook", "", {});
    setGuestbook((_jac.builtin.bool(gb.reports) ? gb.reports[0] : []));
    setGbSigning(false);
    setGraphNonce((graphNonce + 1));
  }
  async function runExplore() {
    setExploring(true);
    let w = await __jacSpawn("explore_graph", "", {});
    setWalkerSnap((_jac.builtin.bool(w.reports) ? w.reports[0] : null));
    setExploring(false);
  }
  async function runStats() {
    setMicroRunning(true);
    setStory("");
    setMicroStats(await __jacCallFunction("guestbook_stats", {}));
    ...
    let acc = "";
    try {
      let resp = await fetch("/function/guestbook_story", {"method": "POST", "headers": {"Content-Type": "application/json"}, "body": "{}"});
      let reader = resp.body.getReader();
      let decoder = Reflect.construct(TextDecoder, []);
      let buf = "";
      let feeding = true;
      while (feeding) {
        ...
        if (part.done) {
          ...
          buf = (buf + _jac.poly.call(decoder, "decode", part.value, {"stream": true}));
          let frames = buf.split(sep);
          buf = frames.pop();
          for (const frame of frames) {
            for (const line of frame.split(nl)) {
              if ((line.startsWith("data: ") && !frame.startsWith("event:"))) {
                acc = (acc + String(JSON.parse(line.slice(6))));
                setStory(acc);
              }
            }
          }
      ...
      if (_jac.exc.matches(__jac_e, "Exception")) {
        let e = __jac_e;
        setStory((acc + " [stream interrupted]"));
      } else {
        throw __jac_e;
      }
    }
  ...
  async function loadSource() {
    setSrcFiles(await __jacCallFunction("list_source_files", {}));
    setSrcLoading(true);
    setSrcFile(await __jacCallFunction("get_source_file", {"path": "frontend.jac"}));
    setSrcLoading(false);
  }
  async function openSource(path) {
    setSrcLoading(true);
    setSrcFile(await __jacCallFunction("get_source_file", {"path": path}));
    setSrcLoading(false);
  }
  return __jacJsx("div", {"className": "min-h-screen bg-background text-foreground"}, [__jacJsx(Nav, {"darkMode": darkMode, "onToggleTheme": () => {
    setDarkMode(!darkMode);
  }}, []), __jacJsx("main", {}, [__jacJsx(HeroSection, {}, []), __jacJsx(InteropSection, {}, []), __jacJsx(LittleXSection, {}, []), __jacJsx(FullstackSection, {"entries": guestbook, "name": gbName, "message": gbMessage, "onNameChange": v => {
    setGbName(v);
  }, "onMessageChange": v => {
    setGbMessage(v);
  }, "onSign": signGuestbook, "signing": gbSigning, "graphRefresh": graphNonce, "walkerSnap": walkerSnap, "exploring": exploring, "onExplore": runExplore}, []), __jacJsx(MicroservicesSection, {"stats": microStats, "running": microRunning, "story": story, "storyOn": storyOn, "onRun": runStats}, []), __jacJsx(NativeSection, {}, []), __jacJsx(WasmShooterSection, {}, []), __jacJsx(ByllmSection, {}, []), __jacJsx(SourceViewer, {"files": srcFiles, "current": srcFile, "loading": srcLoading, "onOpen": openSource}, []), __jacJsx(SeamsSection, {}, []), __jacJsx(OutroSection, {}, [])])]);
}
/*jac:refresh-boundary*/;
export {Showcase, app};
if (typeof globalThis !== "undefined") { if (!globalThis.__jacEndpointEffects__) globalThis.__jacEndpointEffects__ = {}; Object.assign(globalThis.__jacEndpointEffects__, {"func:guestbook_stats": {"is_writer": false, "touches": ["*"]}, "func:list_source_files": {"is_writer": false, "touches": ["*"]}, "func:get_source_file": {"is_writer": false, "touches": ["*"]}}); };
