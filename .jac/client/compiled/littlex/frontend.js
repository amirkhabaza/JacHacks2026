...
function App() {
  ...
  useEffect(() => {
    if (darkMode) {
      _jac.poly.call(document.documentElement.classList, "add", "dark");
    } else {
      _jac.poly.call(document.documentElement.classList, "remove", "dark");
    }
  }, [darkMode]);
  useEffect(() => {
    if (isLoggedIn) {
  ...
  async function handleLike(tweetId) {
    ...
    if (selectedChannel) {
      let detailResult = await __jacSpawn("get_channel_detail", "", {"channel_id": _jac.builtin.jid(selectedChannel.channel)});
  ...
  async function handleSaveBio() {
    let uname = (myProfile ? myProfile.profile.username : "");
  ...
  async function handleJoinChannel(channelId) {
    ...
    let result = await __jacSpawn("get_channels", "", {});
    setChannels((_jac.builtin.bool(result.reports) ? result.reports[0] : []));
    if ((selectedChannel && (_jac.builtin.jid(selectedChannel.channel) === channelId))) {
  ...
  async function handleChannelPost(text) {
    ...
    await __jacSpawn("create_channel_tweet", "", {"channel_id": _jac.builtin.jid(selectedChannel.channel), "content": text});
    let detailResult = await __jacSpawn("get_channel_detail", "", {"channel_id": _jac.builtin.jid(selectedChannel.channel)});
    let detail = (_jac.builtin.bool(detailResult.reports) ? detailResult.reports[0] : null);
    if (detail) {
      setSelectedChannel(detail);
  ...
  let get_tab_name = t => {
    return ((t === "feed") ? "Home" : ((t === "explore") ? "Explore" : ((t === "channels") ? "Channels" : "Profile")));
  };
  if (checkingAuth) {
    return __jacJsx("div", {"className": "flex h-full items-center justify-center bg-background"}, [__jacJsx("div", {"className": "size-8 animate-spin rounded-full border-2 border-primary border-t-transparent"}, [])]);
  }
  if (!isLoggedIn) {
  ...
  let profileUsername = (myProfile ? myProfile.profile.username : "");
  ...
  let myProfileId = (myProfile ? _jac.builtin.jid(myProfile.profile) : "");
  let followingIds = (((myProfile && myProfile.following) ? myProfile.following : [])).map(f => _jac.builtin.jid(f));
  let toFollow = allProfiles.filter(u => (!followingIds.includes(_jac.builtin.jid(u)) && (_jac.builtin.jid(u) !== myProfileId))).map(u => u);
  return __jacJsx("div", {"className": "mx-auto flex h-full w-full max-w-7xl overflow-hidden bg-background text-foreground"}, [__jacJsx(Sidebar, {"activeTab": activeTab, "displayUsername": displayUsername, "profileUsername": profileUsername, "profileAvatar": profileAvatar, "notifCount": notifCount, "onNavigate": navigate, "onLogout": handleLogout}, []), __jacJsx("main", {"className": "h-full min-w-0 flex-1 overflow-y-auto border-x border-border pb-20 md:pb-0"}, [__jacJsx("header", {"className": "sticky top-0 z-30 flex items-center justify-between gap-2 border-b border-border bg-background/80 px-4 py-3 backdrop-blur-lg"}, [__jacJsx("span", {"className": "font-display text-xl font-extrabold"}, [get_tab_name(activeTab)]), __jacJsx("div", {"className": "flex items-center gap-1"}, [(() => {
    let __jac_view_kids = [];
    if ((activeTab === "profile")) {
      __jac_view_kids.push(__jacJsx(Button, {"variant": "outline", "size": "sm", "className": "rounded-full", "onClick": () => {
        setEditingBio(!editingBio);
        setBioText(((myProfile && myProfile.profile.bio) ? myProfile.profile.bio : ""));
      }}, [__jacJsx(Pencil, {"size": 14}, []), (editingBio ? "Cancel" : "Edit profile")]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })(), __jacJsx(Button, {"variant": "ghost", "size": "icon", "className": "rounded-full text-muted-foreground", "title": "Toggle theme", "onClick": () => {
    setDarkMode(!darkMode);
  }}, [(() => {
    let __jac_view_kids_1 = [];
    if (darkMode) {
      __jac_view_kids_1.push(__jacJsx(Sun, {"size": 18}, []));
    } else {
      __jac_view_kids_1.push(__jacJsx(Moon, {"size": 18}, []));
    }
    return __jacJsx(null, {}, [__jac_view_kids_1]);
  })()])])]), (() => {
    let __jac_view_kids_2 = [];
    if ((activeTab === "feed")) {
      __jac_view_kids_2.push(__jacJsx(FeedTab, {"showWelcome": showWelcome, "onDismissWelcome": handleDismissWelcome, "profileAvatar": profileAvatar, "profileUsername": profileUsername, "composerText": composerText, "onComposerChange": v => {
        setComposerText(v);
      ...
      __jac_view_kids_2.push(__jacJsx(ExploreTab, {"allProfiles": allProfiles, "followingIds": followingIds, "myProfileId": myProfileId, "onFollow": handleFollow, "onUnfollow": handleUnfollow}, []));
    } else if ((activeTab === "channels")) {
      __jac_view_kids_2.push(__jacJsx(ChannelsTab, {"channels": channels, "channelsLoading": channelsLoading, "selectedChannel": selectedChannel, "channelPosts": channelPosts, "profileAvatar": profileAvatar, "profileUsername": profileUsername, "onCreateChannel": handleCreateChannel, "onJoinChannel": handleJoinChannel, "onLeaveChannel": handleLeaveChannel, "onSelectChannel": handleSelectChannel, "onChannelPost": handleChannelPost, "onBackToChannels": handleBackToChannels, "onLike": handleLike, "onComment": handleComment, "onDelete": handleDelete, "onRepost": handleRepost}, []));
    } else if ((activeTab === "profile")) {
      __jac_view_kids_2.push(__jacJsx(ProfileTab, {"myProfile": myProfile, "displayUsername": displayUsername, "profileAvatar": profileAvatar, "profileUsername": profileUsername, "editingBio": editingBio, "bioText": bioText, "onBioChange": v => {
        setBioText(v);
    ...
    setSearchQuery("");
    handleSearch("");
  }, "trendingTags": trendingTags, "onTrendClick": handleTrendClick, "toFollow": toFollow, "onFollow": handleFollow, "onShowMore": () => navigate("explore")}, []), __jacJsx(MobileNav, {"activeTab": activeTab, "notifCount": notifCount, "onNavigate": navigate}, []), __jacJsx(Button, {"className": "fixed bottom-20 right-4 z-40 size-14 rounded-full shadow-lg shadow-primary/30 md:hidden", "onClick": () => navigate("feed")}, [__jacJsx(Feather, {"size": 22}, [])])]);
}
const app = App;
