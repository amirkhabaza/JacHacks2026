/* Source: /Users/amirkhabaza/JacHacks2026/littlex/components/AuthForm.jac */
...
function AuthForm(props) {
  ...
  return __jacJsx("div", {"className": "flex h-full w-full bg-background text-foreground"}, [__jacJsx("div", {"className": "relative hidden flex-1 items-center justify-center overflow-hidden bg-gradient-to-br from-primary/20 via-background to-background lg:flex"}, [__jacJsx("div", {"className": "absolute -left-24 top-1/3 size-96 rounded-full bg-primary/30 blur-[120px]"}, []), __jacJsx("div", {"className": "relative flex flex-col items-center gap-6 px-12 text-center"}, [__jacJsx("img", {"src": "/static/assets/littlex-logo.svg", "className": "size-28 drop-shadow-[0_0_40px_var(--primary)]", "alt": "little-x"}, []), __jacJsx("div", {}, [__jacJsx("div", {"className": "font-display text-5xl font-black tracking-tight text-foreground"}, ["LITTLE-X"]), __jacJsx("p", {"className": "mt-3 max-w-sm text-balance text-muted-foreground"}, ["A decentralized social platform built with Jac. Share thoughts, follow people, join the conversation."])])])]), __jacJsx("div", {"className": "flex flex-1 items-center justify-center p-6"}, [__jacJsx(Card, {"className": "w-full max-w-sm border-border/60 shadow-xl"}, [__jacJsx(CardHeader, {"className": "items-center text-center"}, [__jacJsx("img", {"src": "/static/assets/littlex-logo.svg", "className": "mb-2 size-12 lg:hidden", "alt": "little-x"}, []), __jacJsx(CardTitle, {"className": "font-display text-2xl font-extrabold"}, [(isSignup ? "Create your account" : "Welcome back")]), __jacJsx(CardDescription, {}, [(isSignup ? "Join the conversation on Little-X" : "Sign in to your Little-X account")])]), __jacJsx(CardContent, {}, [(() => {
    let __jac_view_kids = [];
    if (error) {
      __jac_view_kids.push(__jacJsx("div", {"className": "mb-4 flex items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive"}, [__jacJsx(AlertCircle, {"size": 16}, []), __jacJsx("span", {}, [error])]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })(), __jacJsx("form", {"onSubmit": onSubmit, "className": "flex flex-col gap-4"}, [__jacJsx(Field, {}, [__jacJsx(FieldLabel, {"htmlFor": "lx-username"}, ["Username"]), __jacJsx(Input, {"id": "lx-username", "type": "text", "placeholder": "yourname", "value": username, "onChange": onUsernameChange, "autoComplete": "username"}, [])]), __jacJsx(Field, {}, [__jacJsx(FieldLabel, {"htmlFor": "lx-password"}, ["Password"]), __jacJsx(Input, {"id": "lx-password", "type": "password", "placeholder": "••••••••", "value": password, "onChange": onPasswordChange, "autoComplete": (isSignup ? "new-password" : "current-password")}, [])]), __jacJsx(Button, {"type": "submit", "className": "mt-1 w-full font-semibold", "disabled": ((loading || !username.trim()) || !password)}, [(loading ? "Processing..." : (isSignup ? "Sign Up" : "Log In"))])]), __jacJsx("div", {"className": "mt-5 text-center text-sm text-muted-foreground"}, [(isSignup ? "Already have an account? " : "New to Little-X? "), __jacJsx("button", {"type": "button", "className": "font-semibold text-primary hover:underline", "onClick": onToggleMode}, [(isSignup ? "Log In" : "Sign Up")])])])])])]);
}
