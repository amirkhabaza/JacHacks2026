/* Source: /Users/amirkhabaza/JacHacks2026/littlex/components/MobileNav.jac */
...
function MobileNav(props) {
  ...
  let item = "flex flex-1 items-center justify-center py-3 transition-colors";
  return __jacJsx("nav", {"className": "fixed inset-x-0 bottom-0 z-40 flex border-t border-border bg-background/80 backdrop-blur-lg md:hidden"}, [__jacJsx("button", {"className": cn(item, ((activeTab === "feed") ? "text-primary" : "text-muted-foreground")), "onClick": () => onNavigate("feed")}, [__jacJsx(Home, {"size": 24}, [])]), __jacJsx("button", {"className": cn(item, ((activeTab === "explore") ? "text-primary" : "text-muted-foreground")), "onClick": () => onNavigate("explore")}, [__jacJsx(Compass, {"size": 24}, [])]), __jacJsx("button", {"className": cn(item, ((activeTab === "channels") ? "text-primary" : "text-muted-foreground")), "onClick": () => onNavigate("channels")}, [__jacJsx(Hash, {"size": 24}, [])]), __jacJsx("button", {"className": cn(item, ((activeTab === "profile") ? "relative text-primary" : "relative text-muted-foreground")), "onClick": () => onNavigate("profile")}, [__jacJsx(User, {"size": 24}, []), (() => {
    let __jac_view_kids = [];
    if ((notifCount > 0)) {
      __jac_view_kids.push(__jacJsx("span", {"className": "absolute right-[28%] top-2 flex size-4 items-center justify-center rounded-full bg-primary text-[0.6rem] font-bold text-primary-foreground"}, [String(notifCount)]));
    }
