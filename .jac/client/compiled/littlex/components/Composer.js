/* Source: /Users/amirkhabaza/JacHacks2026/littlex/components/Composer.jac */
...
function Composer(props) {
  ...
  let ring_color = ((charsLeft < 0) ? "var(--destructive)" : ((charsLeft < 100) ? "var(--primary)" : "var(--muted-foreground)"));
  let over_limit = (showCharRing && (charsLeft < 0));
  let initial = (avatarAlt ? avatarAlt[0].toUpperCase() : "?");
  return __jacJsx("div", {"className": "flex gap-3 border-b border-border px-4 py-3"}, [__jacJsx(Avatar, {"className": "size-10 shrink-0"}, [__jacJsx(AvatarImage, {"src": avatar, "alt": avatarAlt}, []), __jacJsx(AvatarFallback, {}, [initial])]), __jacJsx("div", {"className": "flex min-w-0 flex-1 flex-col gap-2"}, [__jacJsx(Textarea, {"className": "min-h-0 resize-none border-0 bg-transparent px-0 py-1.5 text-lg shadow-none focus-visible:ring-0 dark:bg-transparent", "placeholder": placeholder, "value": value, "onChange": e => onChange(e.target.value), "rows": rows}, []), __jacJsx("div", {"className": "flex items-center justify-end gap-3"}, [(() => {
    let __jac_view_kids = [];
    if ((showCharRing && (value.length > 0))) {
      __jac_view_kids.push(__jacJsx("div", {"className": "flex items-center gap-2"}, [__jacJsx("svg", {"className": "lx-char-ring", "viewBox": "0 0 24 24"}, [__jacJsx("circle", {"className": "lx-char-ring-bg", "cx": 12, "cy": 12, "r": 10}, []), __jacJsx("circle", {"className": "lx-char-ring-fill", "cx": 12, "cy": 12, "r": 10, "strokeDasharray": "62.83", "strokeDashoffset": ring_offset, "stroke": ring_color}, [])]), (() => {
        let __jac_view_kids_1 = [];
        if ((charsLeft <= 20)) {
          __jac_view_kids_1.push(__jacJsx("span", {"className": ("text-xs font-bold tabular-nums " + ((charsLeft < 0) ? "text-destructive" : "text-primary"))}, [String(charsLeft)]));
        }
        return __jacJsx(null, {}, [__jac_view_kids_1]);
      })()]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })(), __jacJsx(Button, {"className": "rounded-full px-5 font-semibold", "disabled": (!value.trim() || over_limit), "onClick": () => onPost()}, [buttonLabel])])])]);
}
