/* Source: /Users/amirkhabaza/JacHacks2026/littlex/components/Sidebar.jac */
...
function Sidebar(props) {
  ...
  let nav_base = "h-12 w-full justify-start gap-4 rounded-full px-4 text-lg font-medium";
  let initial = (displayUsername ? displayUsername[0].toUpperCase() : "?");
  return __jacJsx("aside", {"className": "hidden h-full w-20 shrink-0 flex-col gap-1 overflow-y-auto border-r border-border px-2 py-4 md:flex xl:w-64 xl:px-3"}, [__jacJsx("div", {"className": "mb-2 flex items-center gap-2 px-2 xl:px-3"}, [__jacJsx("img", {"src": "/static/assets/littlex-logo.svg", "className": "size-9", "alt": "little-x"}, []), __jacJsx("span", {"className": "hidden font-display text-xl font-black tracking-tight text-primary xl:inline"}, ["LITTLE-X"])]), __jacJsx("nav", {"className": "flex flex-col gap-1"}, [__jacJsx(Button, {"variant": "ghost", "className": cn(nav_base, ((activeTab === "feed") ? "text-primary bg-primary/10" : "text-foreground")), "onClick": () => onNavigate("feed")}, [__jacJsx(Home, {"size": 24}, []), __jacJsx("span", {"className": "hidden xl:inline"}, ["Home"])]), __jacJsx(Button, {"variant": "ghost", "className": cn(nav_base, ((activeTab === "explore") ? "text-primary bg-primary/10" : "text-foreground")), "onClick": () => onNavigate("explore")}, [__jacJsx(Compass, {"size": 24}, []), __jacJsx("span", {"className": "hidden xl:inline"}, ["Explore"])]), __jacJsx(Button, {"variant": "ghost", "className": cn(nav_base, ((activeTab === "channels") ? "text-primary bg-primary/10" : "text-foreground")), "onClick": () => onNavigate("channels")}, [__jacJsx(Hash, {"size": 24}, []), __jacJsx("span", {"className": "hidden xl:inline"}, ["Channels"])]), __jacJsx(Button, {"variant": "ghost", "className": cn(nav_base, ((activeTab === "profile") ? "relative text-primary bg-primary/10" : "relative text-foreground")), "onClick": () => onNavigate("profile")}, [__jacJsx(User, {"size": 24}, []), __jacJsx("span", {"className": "hidden xl:inline"}, ["Profile"]), (() => {
    let __jac_view_kids = [];
    if ((notifCount > 0)) {
      __jac_view_kids.push(__jacJsx(Badge, {"className": "absolute left-7 top-1.5 size-5 justify-center p-0 text-[0.65rem] xl:left-auto xl:right-4"}, [String(notifCount)]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })()])]), __jacJsx(Button, {"className": "mt-3 h-12 w-12 rounded-full font-semibold shadow-lg shadow-primary/20 xl:w-full", "onClick": () => onNavigate("feed")}, [__jacJsx(Feather, {"size": 20}, []), __jacJsx("span", {"className": "hidden xl:inline"}, ["Post"])]), __jacJsx("div", {"className": "flex-1"}, []), __jacJsx("div", {"className": "flex items-center gap-2 rounded-full p-2"}, [__jacJsx(Avatar, {"className": "size-9 shrink-0"}, [__jacJsx(AvatarImage, {"src": profileAvatar, "alt": profileUsername}, []), __jacJsx(AvatarFallback, {}, [initial])]), __jacJsx("div", {"className": "hidden min-w-0 flex-1 text-left xl:block"}, [__jacJsx("div", {"className": "truncate text-sm font-semibold text-foreground"}, [displayUsername]), __jacJsx("div", {"className": "truncate text-xs text-muted-foreground"}, [("@" + displayUsername)])]), __jacJsx(Button, {"variant": "ghost", "size": "icon", "className": "ml-auto shrink-0 rounded-full text-muted-foreground hover:text-destructive", "title": ("Log out @" + displayUsername), "onClick": () => onLogout()}, [__jacJsx(LogOut, {"size": 18}, [])])])]);
}
