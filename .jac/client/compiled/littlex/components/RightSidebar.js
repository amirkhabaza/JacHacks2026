/* Source: /Users/amirkhabaza/JacHacks2026/littlex/components/RightSidebar.jac */
...
import { HoverCard, HoverCardTrigger, HoverCardContent } from "./ui/hover_card.js";
const _jac = {
  int: {
    bit_length(n) {
      if (n === 0) return 0;
...
function RightSidebar(props) {
  ...
  return __jacJsx("aside", {"className": "hidden h-full w-80 shrink-0 flex-col gap-4 overflow-y-auto border-l border-border px-4 py-4 lg:flex"}, [__jacJsx("div", {"className": "relative"}, [__jacJsx(Search, {"size": 16, "className": "pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground"}, []), __jacJsx(Input, {"className": "rounded-full bg-muted pl-9 pr-9", "placeholder": "Search Little-X", "value": searchQuery, "onChange": e => onSearchChange(e.target.value), "onKeyDown": e => {
    if ((e.key === "Enter")) {
    ...
    let __jac_view_kids = [];
    if (searchQuery) {
      __jac_view_kids.push(__jacJsx("button", {"className": "absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground", "title": "Clear search", "onClick": () => onClearSearch()}, [__jacJsx(X, {"size": 16}, [])]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })()]), (() => {
    let __jac_view_kids_1 = [];
    if ((trendingTags.length > 0)) {
      __jac_view_kids_1.push(__jacJsx(Card, {"className": "gap-0 overflow-hidden py-0"}, [__jacJsx(CardHeader, {"className": "border-b border-border py-3"}, [__jacJsx(CardTitle, {"className": "flex items-center gap-2 text-base font-display"}, [__jacJsx(TrendingUp, {"size": 16, "className": "text-primary"}, []), "Trending"])]), __jacJsx(CardContent, {"className": "px-0 py-1"}, [(() => {
        let __jac_view_kids_2 = [];
        for (const t of trendingTags) {
          __jac_view_kids_2.push(__jacJsx("button", {"className": "flex w-full flex-col items-start px-4 py-2 text-left transition-colors hover:bg-muted/50", "key": t.tag, "onClick": () => onTrendClick(t.tag)}, [__jacJsx("span", {"className": "font-semibold text-primary"}, [t.tag]), __jacJsx("span", {"className": "text-xs text-muted-foreground"}, [(String(t.count) + ((t.count === 1) ? " post" : " posts"))])]));
        }
    ...
    if ((toFollow.length > 0)) {
      __jac_view_kids_3.push(__jacJsx(Card, {"className": "gap-0 overflow-hidden py-0"}, [__jacJsx(CardHeader, {"className": "border-b border-border py-3"}, [__jacJsx(CardTitle, {"className": "text-base font-display"}, ["Who to follow"])]), __jacJsx(CardContent, {"className": "px-0 py-1"}, [(() => {
        let __jac_view_kids_4 = [];
        for (const u of toFollow.slice(0, 5)) {
          __jac_view_kids_4.push(__jacJsx("div", {"className": "flex items-center gap-2 px-4 py-2 transition-colors hover:bg-muted/50", "key": _jac.builtin.jid(u)}, [__jacJsx(Avatar, {"className": "size-9 shrink-0"}, [__jacJsx(AvatarImage, {"src": ("https://i.pravatar.cc/150?u=fake@" + u.username), "alt": u.username}, []), __jacJsx(AvatarFallback, {}, [(u.username ? ((u.username.includes("@") ? u.username.split("@")[0] : u.username))[0].toUpperCase() : "?")])]), __jacJsx("div", {"className": "min-w-0 flex-1"}, [__jacJsx(HoverCard, {}, [__jacJsx(HoverCardTrigger, {"asChild": true}, [__jacJsx("span", {"className": "block cursor-pointer truncate text-sm font-semibold text-foreground hover:underline"}, [(u.username.includes("@") ? u.username.split("@")[0] : u.username)])]), __jacJsx(HoverCardContent, {"className": "w-64"}, [__jacJsx("div", {"className": "flex items-center gap-3"}, [__jacJsx(Avatar, {"className": "size-11"}, [__jacJsx(AvatarImage, {"src": ("https://i.pravatar.cc/150?u=fake@" + u.username), "alt": u.username}, []), __jacJsx(AvatarFallback, {}, [(u.username ? ((u.username.includes("@") ? u.username.split("@")[0] : u.username))[0].toUpperCase() : "?")])]), __jacJsx("div", {"className": "min-w-0"}, [__jacJsx("div", {"className": "truncate font-semibold text-foreground"}, [(u.username.includes("@") ? u.username.split("@")[0] : u.username)]), __jacJsx("div", {"className": "truncate text-xs text-muted-foreground"}, [("@" + (u.username.includes("@") ? u.username.split("@")[0] : u.username))])])]), __jacJsx("p", {"className": "mt-2 line-clamp-3 text-sm text-muted-foreground"}, [(u.bio || "No bio yet")])])]), __jacJsx("div", {"className": "truncate text-xs text-muted-foreground"}, [("@" + (u.username.includes("@") ? u.username.split("@")[0] : u.username))])]), __jacJsx(Button, {"size": "sm", "className": "shrink-0 rounded-full font-semibold", "onClick": () => onFollow(_jac.builtin.jid(u))}, ["Follow"])]));
        }
        return __jacJsx(null, {}, [__jac_view_kids_4]);
      })(), (() => {
        let __jac_view_kids_5 = [];
        if ((toFollow.length > 5)) {
          __jac_view_kids_5.push(__jacJsx("button", {"className": "w-full px-4 py-2 text-left text-sm text-primary hover:bg-muted/50", "onClick": () => onShowMore()}, ["Show more"]));
        }
        return __jacJsx(null, {}, [__jac_view_kids_5]);
      })()])]));
    }
    return __jacJsx(null, {}, [__jac_view_kids_3]);
  })(), __jacJsx(Card, {"className": "bg-muted/30"}, [__jacJsx(CardHeader, {}, [__jacJsx(CardTitle, {"className": "font-display text-base text-primary"}, ["LITTLE-X"])]), __jacJsx(CardContent, {"className": "-mt-3"}, [__jacJsx("p", {"className": "text-sm text-muted-foreground"}, ["A decentralized social platform built with Jac. Share thoughts, follow people, join conversations."]), __jacJsx("div", {"className": "mt-3 flex flex-wrap gap-1.5"}, [__jacJsx(Badge, {"variant": "secondary"}, ["#jacdev"]), __jacJsx(Badge, {"variant": "secondary"}, ["#jaseci"]), __jacJsx(Badge, {"variant": "secondary"}, ["#open-source"])])])])]);
}
