/* Source: /Users/amirkhabaza/JacHacks2026/littlex/components/ExploreTab.jac */
...
function ExploreTab(props) {
  ...
  return __jacJsx("div", {}, [(() => {
    ...
    if (!_jac.builtin.bool(allProfiles)) {
      __jac_view_kids.push(__jacJsx("div", {"className": "flex flex-col gap-4 p-4"}, [_jac.builtin.range(5).map(i => __jacJsx("div", {"className": "flex items-center gap-3", "key": i}, [__jacJsx(Skeleton, {"className": "size-11 rounded-full"}, []), __jacJsx("div", {"className": "flex-1 space-y-2"}, [__jacJsx(Skeleton, {"className": "h-3 w-28"}, []), __jacJsx(Skeleton, {"className": "h-3 w-40"}, [])]), __jacJsx(Skeleton, {"className": "h-9 w-20 rounded-full"}, [])]))]));
    }
    for (const u of allProfiles) {
      __jac_view_kids.push(__jacJsx("div", {"className": "lx-fade-up flex items-center gap-3 border-b border-border px-4 py-3 transition-colors hover:bg-muted/30", "key": _jac.builtin.jid(u)}, [__jacJsx(Avatar, {"className": "size-11 shrink-0"}, [__jacJsx(AvatarImage, {"src": ("https://i.pravatar.cc/150?u=fake@" + u.username), "alt": u.username}, []), __jacJsx(AvatarFallback, {}, [(u.username ? ((u.username.includes("@") ? u.username.split("@")[0] : u.username))[0].toUpperCase() : "?")])]), __jacJsx("div", {"className": "min-w-0 flex-1"}, [__jacJsx("div", {"className": "font-display font-bold text-foreground truncate"}, [(u.username.includes("@") ? u.username.split("@")[0] : u.username)]), __jacJsx("div", {"className": "text-sm text-muted-foreground truncate"}, [("@" + (u.username.includes("@") ? u.username.split("@")[0] : u.username))]), __jacJsx("div", {"className": "mt-0.5 line-clamp-2 text-sm text-foreground/80"}, [(u.bio || "No bio yet")])]), (() => {
        let __jac_view_kids_1 = [];
        if (followingIds.includes(_jac.builtin.jid(u))) {
          __jac_view_kids_1.push(__jacJsx(Button, {"variant": "outline", "size": "sm", "className": "group shrink-0 rounded-full hover:border-destructive/40 hover:bg-destructive/10 hover:text-destructive", "onClick": () => onUnfollow(_jac.builtin.jid(u))}, [__jacJsx("span", {"className": "group-hover:hidden"}, ["Following"]), __jacJsx("span", {"className": "hidden group-hover:inline"}, ["Unfollow"])]));
        } else if ((_jac.builtin.jid(u) !== myProfileId)) {
          __jac_view_kids_1.push(__jacJsx(Button, {"size": "sm", "className": "shrink-0 rounded-full font-semibold", "onClick": () => onFollow(_jac.builtin.jid(u))}, ["Follow"]));
        }
    ...
    if (_jac.builtin.bool(_jac.poly.and_(allProfiles, () => (allProfiles.length === 0)))) {
      __jac_view_kids_2.push(__jacJsx(Empty, {"className": "py-16"}, [__jacJsx(EmptyHeader, {}, [__jacJsx(EmptyMedia, {"variant": "icon"}, [__jacJsx(Users, {}, [])]), __jacJsx(EmptyTitle, {}, ["No one to show"]), __jacJsx(EmptyDescription, {}, ["Check back later to discover more people."])])]));
    }
