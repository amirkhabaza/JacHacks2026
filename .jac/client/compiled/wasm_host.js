/* Source: /Users/amirkhabaza/.cache/jac/rt/13d383ad3c816638-2e8a5ffcbb4cfb90/site/jaclang/runtimelib/wasm_host.cl.jac */
...
class _HostState {
  constructor(props = {}) {
    this.sh = (props.hasOwnProperty("sh") ? props.sh : null);
    this.out1 = (props.hasOwnProperty("out1") ? props.out1 : "");
    this.out2 = (props.hasOwnProperty("out2") ? props.out2 : "");
  }
}
function _emit(st, fd, text) {
  if ((fd === 2)) {
    st.out2 = (st.out2 + text);
  } else {
    st.out1 = (st.out1 + text);
  }
  let buf = ((fd === 2) ? st.out2 : st.out1);
  let idx = buf.lastIndexOf("\n");
  if ((idx >= 0)) {
    let head = buf.slice(0, idx);
    let tail = buf.slice((idx + 1));
    if ((fd === 2)) {
      console.error(head);
      st.out2 = tail;
    } else {
...
function _clock_gettime(st, clk, ts) {
  let dv = Reflect.construct(DataView, [st.sh.mem.buffer]);
  let ms = ((clk === 0) ? Date.now() : performance.now());
  let sec = Math.floor((ms / 1000.0));
  let nsec = Math.floor(((ms - (sec * 1000.0)) * 1000000.0));
  dv.setBigInt64(ts, BigInt(sec), true);
  dv.setBigInt64((ts + 8), BigInt(nsec), true);
  return 0;
}
function make_jac_host(sh) {
  let st = new _HostState({sh: sh});
  return {"write": (fd, ptr, n) => {
    let bytes_view = Reflect.construct(Uint8Array, [st.sh.mem.buffer, ptr, n]);
    _emit(st, fd, _jac.poly.call(Reflect.construct(TextDecoder, []), "decode", bytes_view));
    return n;
  }, "fflush": f => {
    _flush_all(st);
    return 0;
    ...
    _flush_all(st);
    throw new Error((("na wasm module called exit(" + String(code)) + ")"));
  }, "abort": () => {
    _flush_all(st);
    throw new Error("na wasm module called abort()");
  }, "getenv": p => {
    return 0;
  }, "fopen": (p, m) => {
    return 0;
    ...
    return _clock_gettime(st, clk, ts);
  }, "nanosleep": (req, rem) => {
    return 0;
  }};
}
async function instantiate_na_wasm(url, sh, extra_imports = null) {
  let imports = Object.assign({}, (extra_imports || {}));
  imports["jac_host1"] = make_jac_host(sh);
  let res = await WebAssembly.instantiateStreaming(fetch(url), imports);
...
let _na_env_cfg = {};
let _na_pending = {};
function set_na_env(module_stem, sh, extra_imports = null) {
  _na_env_cfg[module_stem] = [sh, extra_imports];
}
async function _na_instance(module_stem) {
  if ((module_stem in _na_pending)) {
    return await _na_pending[module_stem];
  }
  let cfg = (_na_env_cfg[module_stem] ?? undefined);
  let sh = (cfg ? cfg[0] : {});
  let extra = (cfg ? cfg[1] : null);
  let p = instantiate_na_wasm(`/static/${module_stem}.wasm`, sh, extra);
  ...
  try {
    ...
    if (_jac.exc.matches(__jac_e, "Exception")) {
      let err = __jac_e;
      _jac.dict.pop(_na_pending, module_stem, null);
      throw err;
    } else {
      throw __jac_e;
    }
...
function _na_caller(module_stem, fn_name) {
  async function _call(...args) {
    let inst = await _na_instance(module_stem);
    let fn = _jac.poly.getitem(inst.exports, fn_name);
    if (!fn) {
      throw new Error(`na module '${module_stem} has no export ${fn_name}`);
    }
    return fn(...args);
  }
  return _call;
}
function __na_bind(module_stem, names) {
  let out = {};
...
export {__na_bind, instantiate_na_wasm, make_jac_host, set_na_env};
