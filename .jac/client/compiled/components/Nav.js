/* Source: /Users/amirkhabaza/JacHacks2026/components/Nav.jac */
...
let _LINKS = [{"href": "#interop", "label": "Interop"}, {"href": "#littlex", "label": "littleX"}, {"href": "#fullstack", "label": "Fullstack"}, {"href": "#microservices", "label": "Microservices"}, {"href": "#native", "label": "Native"}, {"href": "#wasm", "label": "WebAssembly"}, {"href": "#ai", "label": "AI-native"}, {"href": "#source", "label": "Source"}, {"href": "#seams", "label": "Cohesion"}];
function Nav(props) {
  const {darkMode, onToggleTheme} = props;
  return __jacJsx("header", {"className": "fixed inset-x-0 top-0 z-50 border-b border-border/60 bg-background/70 backdrop-blur-xl"}, [__jacJsx("nav", {"className": "mx-auto flex h-16 w-full max-w-6xl items-center gap-6 px-6"}, [__jacJsx("a", {"href": "#top", "className": "flex items-center gap-2 font-display font-bold"}, [__jacJsx("span", {"className": "flex size-7 items-center justify-center rounded-lg bg-primary font-mono text-sm text-primary-foreground"}, ["J"]), __jacJsx("span", {"className": "text-foreground"}, ["this is ", __jacJsx("span", {"className": "tij-gradient-text"}, ["jac"])])]), __jacJsx("div", {"className": "ml-4 hidden items-center gap-1 md:flex"}, [_LINKS.map(link => __jacJsx("a", {"href": link["href"], "key": link["href"], "className": "rounded-md px-3 py-1.5 text-sm text-muted-foreground transition hover:bg-muted hover:text-foreground"}, [link["label"]]))]), __jacJsx("div", {"className": "ml-auto flex items-center gap-1"}, [__jacJsx(Button, {"variant": "ghost", "size": "icon", "className": "rounded-full text-muted-foreground", "title": "Toggle theme", "onClick": () => onToggleTheme()}, [(() => {
    let __jac_view_kids = [];
    if (darkMode) {
      __jac_view_kids.push(__jacJsx(Sun, {"size": 18}, []));
    } else {
      __jac_view_kids.push(__jacJsx(Moon, {"size": 18}, []));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })()]), __jacJsx("a", {"href": "https://github.com/jaseci-labs/jaseci", "target": "_blank", "rel": "noreferrer"}, [__jacJsx(Button, {"variant": "ghost", "size": "icon", "className": "rounded-full text-muted-foreground", "title": "GitHub"}, [__jacJsx(Github, {"size": 18}, [])])])])])]);
}
