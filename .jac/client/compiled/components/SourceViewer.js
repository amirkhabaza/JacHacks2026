/* Source: /Users/amirkhabaza/JacHacks2026/components/SourceViewer.jac */
import {__jacJsx, __jacSpawn} from "@jac/runtime";
import { Search, Copy, Check, FileCode2, Folder, FolderOpen, Code2, ChevronDown, ChevronRight } from "lucide-react";
...
let _ROOT_LABEL = "this_is_jac";
function _lang_color(lang) {
  if ((lang === "Jac")) {
    return "text-primary";
  }
  if ((lang === "Python")) {
...
function SourceViewer(props) {
  const {files, current, loading, onOpen} = props;
  ...
  let filtered = files.filter(f => ((q === "") || _jac.poly.contains(_jac.poly.call(f["path"], "lower"), q))).map(f => f);
  let buckets = {};
  let dirset = {};
  for (const f of filtered) {
    ...
    _jac.poly.call(buckets[d], "append", f);
    while ((d !== "")) {
      dirset[d] = true;
      d = (_jac.poly.contains(d, "/") ? _jac.poly.call(d, "rsplit", "/", 1)[0] : "");
    }
  }
  let coll = ((q !== "") ? {} : collapsed);
  let rows = [];
  rows.push({"kind": "dir", "path": "", "name": _ROOT_LABEL, "depth": 0});
  if (!(coll[""] ?? false)) {
    for (const f of (buckets[""] ?? [])) {
      rows.push({"kind": "file", "file": f, "depth": 1});
    }
    for (const d of _jac.builtin.sorted(Object.keys(dirset))) {
      let hidden = false;
      let a = (_jac.poly.contains(d, "/") ? _jac.poly.call(d, "rsplit", "/", 1)[0] : "");
      while ((a !== "")) {
        if ((coll[a] ?? false)) {
          hidden = true;
        }
        a = (_jac.poly.contains(a, "/") ? _jac.poly.call(a, "rsplit", "/", 1)[0] : "");
      }
      if (hidden) {
        continue;
      }
      let depth = (_jac.poly.call(d, "count", "/") + 1);
      rows.push({"kind": "dir", "path": d, "name": _jac.poly.getitem(_jac.poly.call(d, "rsplit", "/", 1), (-1)), "depth": depth});
      if (!(coll[d] ?? false)) {
        for (const f of (buckets[d] ?? [])) {
          rows.push({"kind": "file", "file": f, "depth": (depth + 1)});
        }
      }
    }
  }
  let toggleDir = d => {
    let nc = {...collapsed};
    nc[d] = !(collapsed[d] ?? false);
    setCollapsed(nc);
  };
  let curPath = (current ? current["path"] : "");
  let curLang = (current ? current["lang"] : "");
  let curHtml = (current ? current["html"] : "");
  let curErr = (current ? current["error"] : "");
  let curLines = (current ? current["lines"] : 0);
  let curRaw = (current ? current["raw"] : "");
  let do_copy = () => {
    navigator.clipboard.writeText(curRaw);
    setCopied(true);
  };
  return __jacJsx(SectionShell, {"id": "source", "eyebrow": "Open source", "title": "This Demo is One Typechecked Jac Program. Let Me Prove It.", "subtitle": "Every pixel you have scrolled is Jac - client, server, and native, in one project. Browse the actual files below, I'd recommend checking out analytics.jac, main.jac, social_graph.jac, etc."}, [__jacJsx("div", {"className": "mx-auto max-w-6xl"}, [__jacJsx("div", {"className": "dark grid overflow-hidden rounded-2xl border border-border bg-card text-foreground shadow-xl shadow-black/30 lg:grid-cols-[290px_1fr]"}, [__jacJsx("div", {"className": "flex max-h-[600px] min-h-[440px] flex-col border-b border-border lg:border-b-0 lg:border-r"}, [__jacJsx("div", {"className": "flex items-center gap-2 border-b border-border/70 px-3 py-2.5"}, [__jacJsx(Search, {"size": 14, "className": "shrink-0 text-muted-foreground"}, []), __jacJsx("input", {"value": query, "onChange": e => {
    setQuery(e.target.value);
  }, "placeholder": "Filter files...", "className": "w-full bg-transparent text-sm text-foreground placeholder:text-muted-foreground focus:outline-none"}, []), __jacJsx("span", {"className": "shrink-0 font-mono text-[10px] text-muted-foreground"}, [filtered.length])]), __jacJsx("div", {"className": "flex-1 overflow-y-auto py-1"}, [rows.map(r => ((r["kind"] === "dir") ? __jacJsx("button", {"key": ("d:" + r["path"]), "onClick": () => toggleDir(r["path"]), "className": "flex w-full items-center gap-1.5 px-3 py-1 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70 transition hover:bg-white/5 hover:text-foreground"}, [_jac.builtin.range(r["depth"]).map(i => __jacJsx("span", {"key": i, "className": "w-3 shrink-0"}, [])), (() => {
    let __jac_view_kids = [];
    if ((coll[r["path"]] ?? false)) {
      __jac_view_kids.push(__jacJsx(ChevronRight, {"size": 11, "className": "shrink-0"}, []));
    } else {
      __jac_view_kids.push(__jacJsx(ChevronDown, {"size": 11, "className": "shrink-0"}, []));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })(), (() => {
    let __jac_view_kids_1 = [];
    if ((coll[r["path"]] ?? false)) {
      __jac_view_kids_1.push(__jacJsx(Folder, {"size": 11, "className": "shrink-0"}, []));
    } else {
      __jac_view_kids_1.push(__jacJsx(FolderOpen, {"size": 11, "className": "shrink-0"}, []));
    }
    return __jacJsx(null, {}, [__jac_view_kids_1]);
  })(), __jacJsx("span", {"className": "truncate"}, [r["name"]])]) : __jacJsx("button", {"key": r["file"]["path"], "onClick": () => {
    setActive(r["file"]["path"]);
    onOpen(r["file"]["path"]);
  }, "className": ("flex w-full items-center gap-2 px-3 py-1 text-left text-[13px] transition " + ((r["file"]["path"] === active) ? "bg-primary/15 text-foreground" : "text-muted-foreground hover:bg-white/5 hover:text-foreground"))}, [_jac.builtin.range(r["depth"]).map(i => __jacJsx("span", {"key": i, "className": "w-3 shrink-0"}, [])), __jacJsx(FileCode2, {"size": 13, "className": ("shrink-0 " + ((r["file"]["path"] === active) ? _lang_color(r["file"]["lang"]) : "text-muted-foreground/50"))}, []), __jacJsx("span", {"className": "truncate"}, [r["file"]["name"]])])))])]), __jacJsx("div", {"className": "flex max-h-[600px] min-w-0 flex-col bg-[#0c0e14]"}, [__jacJsx("div", {"className": "flex items-center gap-2 border-b border-border/70 px-4 py-2.5"}, [__jacJsx("span", {"className": "size-3 shrink-0 rounded-full bg-[#ff5f57]"}, []), __jacJsx("span", {"className": "size-3 shrink-0 rounded-full bg-[#febc2e]"}, []), __jacJsx("span", {"className": "size-3 shrink-0 rounded-full bg-[#28c840]"}, []), __jacJsx("span", {"className": "ml-2 truncate font-mono text-xs text-muted-foreground"}, [(curPath || "\u2014")]), (() => {
    let __jac_view_kids_2 = [];
    if (curLang) {
      __jac_view_kids_2.push(__jacJsx("span", {"className": ("ml-1 shrink-0 rounded-md border border-border/60 px-1.5 py-0.5 font-mono text-[10px] " + _lang_color(curLang))}, [curLang]));
    }
    return __jacJsx(null, {}, [__jac_view_kids_2]);
  })(), __jacJsx("button", {"className": "ml-auto flex shrink-0 items-center gap-1.5 rounded-md px-2 py-1 text-xs text-muted-foreground transition hover:bg-white/5 hover:text-foreground", "onClick": do_copy, "title": "Copy file"}, [(() => {
    let __jac_view_kids_3 = [];
    if (copied) {
      __jac_view_kids_3.push(__jacJsx(Check, {"size": 13, "className": "text-emerald-400"}, []));
    } else {
      __jac_view_kids_3.push(__jacJsx(Copy, {"size": 13}, []));
    }
    return __jacJsx(null, {}, [__jac_view_kids_3]);
  })(), (copied ? "Copied" : "Copy")])]), __jacJsx("div", {"className": "relative flex-1 overflow-y-auto"}, [(() => {
    let __jac_view_kids_4 = [];
    if (curErr) {
      __jac_view_kids_4.push(__jacJsx("div", {"className": "p-8 text-center text-sm text-muted-foreground"}, [curErr]));
    } else if (curHtml) {
      __jac_view_kids_4.push(__jacJsx("div", {"className": "flex min-h-full"}, [__jacJsx("div", {"className": "select-none border-r border-border/40 px-3 py-3 text-right font-mono text-[12.5px] leading-[1.55] text-muted-foreground/40"}, [_jac.builtin.range(1, (curLines + 1)).map(i => __jacJsx("div", {"key": i}, [i]))]), __jacJsx("pre", {"className": "tij-code flex-1 overflow-x-auto px-4 py-3 font-mono text-[12.5px] leading-[1.55]"}, [_jac.builtin.unsafe_html(curHtml)])]));
    } else {
      __jac_view_kids_4.push(__jacJsx("div", {"className": "flex h-full items-center justify-center gap-2 p-8 text-sm text-muted-foreground"}, [__jacJsx(Code2, {"size": 16}, []), "Select a file to view its source"]));
    }
    return __jacJsx(null, {}, [__jac_view_kids_4]);
  })(), (() => {
    let __jac_view_kids_5 = [];
    if (loading) {
      __jac_view_kids_5.push(__jacJsx("div", {"className": "absolute inset-0 flex items-center justify-center bg-[#0c0e14]/60 backdrop-blur-[1px]"}, [__jacJsx("div", {"className": "size-5 animate-spin rounded-full border-2 border-primary border-t-transparent"}, [])]));
    }
...
export {SourceViewer};
//# sourceMappingURL=SourceViewer.js.map
