/* Source: /Users/amirkhabaza/JacHacks2026/components/MicroservicesSection.jac */
import {__jacJsx, __jacSpawn} from "@jac/runtime";
import { Cpu, Server, ArrowRight, Boxes, Play, Radio } from "lucide-react";
...
let _CONSUMER = "# guestbook.jac - serves its walkers to the browser AND\n# consumes a microservice. One relative sv import is the whole integration.\nsv import from .analytics { analyze, narrate }\n\ndef:pub guestbook_stats() -> dict {\n    rep = analyze(count_signatures());      # -> SEPARATE service\n    return {\n        \"gateway_pid\": os.getpid(),         # this server\n        \"analytics_pid\": rep.pid,           # the analytics microservice\n        \"score\": rep.score                  # typed, end to end\n    };\n}\n\n# The SAME import also streams: the sv->sv call returns a LIVE\n# generator (one SSE frame per yield) - re-yield it to the browser.\ndef:pub guestbook_story() -> Generator {\n    for chunk in narrate(count_signatures()) {\n        yield chunk;                        # frame in, frame out\n    }\n}";
let _PROVIDER = "# analytics.jac - the provider service.\nimport os;\nobj AnalyticsReport { has pid: int, score: float; }\n\ndef:pub analyze(signatures: int) -> AnalyticsReport {\n    # runs in analytics' OWN service process - pid proves it\n    return AnalyticsReport(pid=os.getpid(), score=float(signatures) * 1.5);\n}\n\n# A generator def:pub IS a streaming endpoint: every yield leaves\n# this process as one server-sent event.\ndef:pub narrate(signatures: int) -> Generator {\n    for word in commentary(signatures).split(\" \") {\n        yield word + \" \";                   # one SSE frame per word\n    }\n}";
function ProcBox(props) {
  const {label, sub, pid, icon, accent} = props;
  return __jacJsx("div", {"className": ("flex flex-1 flex-col items-center gap-1 rounded-xl border p-4 text-center " + (accent ? "border-primary/40 bg-primary/10" : "border-border bg-card/60"))}, [__jacJsx("div", {"className": "flex items-center gap-2 text-sm font-semibold text-foreground"}, [icon, label]), __jacJsx("div", {"className": "font-mono text-xs text-muted-foreground"}, ["pid", " ", __jacJsx("span", {"className": "text-primary"}, [pid])]), __jacJsx("div", {"className": "font-mono text-[0.65rem] text-muted-foreground/70"}, [sub])]);
}
function MicroservicesSection(props) {
  const {stats, running, story, storyOn, onRun} = props;
  let gpid = (stats ? String(stats["gateway_pid"]) : "\u2014");
  let apid = (stats ? String(stats["analytics_pid"]) : "\u2014");
  return __jacJsx(SectionShell, {"id": "microservices", "eyebrow": "Microservices", "title": "One import is a Microservice boundary", "subtitle": "One relative `sv import from .analytics` specifies a microservice. Done. Typechecked across. The runtime spawns analytics as its own service and routes the call - the compiler generates HTTP APIs, serializers, service URLs, etc. This is real. Click the button. Watch the same import STREAM, and the analytics commentary arrives word by word over SSE."}, [__jacJsx("div", {"className": "mx-auto max-w-5xl"}, [__jacJsx("div", {"className": "grid items-stretch gap-4 lg:grid-cols-2"}, [__jacJsx(CodeBlock, {"filename": "guestbook.jac", "code": _CONSUMER, "dense": true}, []), __jacJsx(CodeBlock, {"filename": "analytics.jac", "code": _PROVIDER, "dense": true}, [])]), __jacJsx("div", {"className": "mt-4 rounded-xl border border-border bg-card/60 p-6"}, [__jacJsx("div", {"className": "mb-4 flex items-center justify-between"}, [__jacJsx("span", {"className": "flex items-center gap-2 text-sm font-medium text-foreground"}, [__jacJsx(Boxes, {"size": 15, "className": "text-primary"}, []), "Live cross-process call"]), __jacJsx(Button, {"size": "sm", "className": "gap-1.5 rounded-full", "disabled": running, "onClick": () => onRun()}, [__jacJsx(Play, {"size": 13}, []), (running ? "Calling..." : "Run guestbook_stats")])]), __jacJsx("div", {"className": "flex items-center gap-2"}, [__jacJsx(ProcBox, {"label": "guestbook", "sub": "the showcase server", "pid": gpid, "icon": __jacJsx(Server, {"size": 15, "className": "text-primary"}, []), "accent": false}, []), __jacJsx("div", {"className": "flex shrink-0 flex-col items-center text-muted-foreground"}, [__jacJsx(ArrowRight, {"size": 20}, []), __jacJsx("span", {"className": "font-mono text-[0.65rem]"}, ["sv import"])]), __jacJsx(ProcBox, {"label": "analytics", "sub": "auto-spawned microservice", "pid": apid, "icon": __jacJsx(Cpu, {"size": 15, "className": "text-primary"}, []), "accent": true}, [])]), (() => {
    let __jac_view_kids = [];
    if (stats) {
      __jac_view_kids.push(__jacJsx("div", {"className": "mt-4 rounded-lg border border-border bg-background/50 p-3 text-sm"}, [__jacJsx("div", {"className": "text-muted-foreground"}, ["analytics computed a reach score of", " ", __jacJsx("span", {"className": "font-mono font-bold text-primary"}, [stats["score"]]), " ", "over", " ", __jacJsx("span", {"className": "font-mono text-foreground"}, [stats["signatures"]]), " ", "guestbook signatures."]), __jacJsx("div", {"className": "mt-1 text-xs text-muted-foreground"}, [((stats["gateway_pid"] !== stats["analytics_pid"]) ? "Two different PIDs - analytics ran in a separate process the runtime spawned from one sv import." : "Same PID (in-process).")])]));
    } else {
      __jac_view_kids.push(__jacJsx("div", {"className": "mt-4 rounded-lg border border-border bg-background/50 p-3 text-center text-sm text-muted-foreground"}, ["Run it - the guestbook server forwards your live signature count to the analytics microservice and reports both process ids."]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })(), (() => {
    let __jac_view_kids_1 = [];
    if ((storyOn || (story !== ""))) {
      __jac_view_kids_1.push(__jacJsx("div", {"className": "mt-4 rounded-lg border border-primary/30 bg-primary/[0.05] p-4"}, [__jacJsx("div", {"className": "mb-2 flex items-center gap-1.5 font-mono text-[10px] font-semibold uppercase tracking-wider text-primary"}, [__jacJsx(Radio, {"size": 13, "className": (storyOn ? "animate-pulse" : "")}, []), "streaming live from the analytics process - one SSE frame per word"]), __jacJsx("p", {"className": "text-sm leading-relaxed text-foreground/85"}, [story, (() => {
        let __jac_view_kids_2 = [];
        if (storyOn) {
          __jac_view_kids_2.push(__jacJsx("span", {"className": "animate-pulse text-primary"}, ["▍"]));
        }
...
export {MicroservicesSection, ProcBox};
//# sourceMappingURL=MicroservicesSection.js.map
