/* Source: /Users/amirkhabaza/JacHacks2026/components/FullstackSection.jac */
import {__jacJsx, __jacSpawn} from "@jac/runtime";
import { Send, MessageSquare, Database, Footprints, Play } from "lucide-react";
...
function _wlayout(snap) {
  ...
  let rows = Math.max(vis.length, days.length, 1);
  let h = (70 + (rows * 46));
  let pos = {};
  for (const i of _jac.builtin.range(vis.length)) {
    pos[vis.at(i).id] = {"x": 430.0, "y": (56.0 + (i * 46))};
  }
  for (const i of _jac.builtin.range(days.length)) {
    let ys = [];
    for (const e of snap.edges) {
      if (((e[0] === days.at(i).id) && (e[1] in pos))) {
        ys.push(pos[e[1]]["y"]);
      }
    }
    let dy = (_jac.builtin.bool(ys) ? (_jac.builtin.sum(ys) / ys.length) : (56.0 + (i * 46)));
    pos[days.at(i).id] = {"x": 255.0, "y": dy};
  }
  for (const n of snap.nodes) {
    if ((n.kind === "root")) {
...
function FullstackSection(props) {
  const {entries, name, message, onNameChange, onMessageChange, onSign, signing, graphRefresh: graphRefresh = 0, walkerSnap: walkerSnap = null, exploring: exploring = false, onExplore: onExplore = null} = props;
  let ly = (walkerSnap ? _wlayout(walkerSnap) : {});
  return __jacJsx(SectionShell, {"id": "fullstack", "eyebrow": "Fullstack", "title": "Introducing a New Paradigm, for a New Way of Problem Solving.", "subtitle": "Use graph centric PL semantics to solve problems in an intuitive and elegant way. Sign below and it's persisted to a real graph. (and yes, this is a secret weapon for agentic ai harness building, but that's for another demo ;-))"}, [__jacJsx("div", {}, [__jacJsx("div", {"className": "grid items-start gap-8 lg:grid-cols-2"}, [__jacJsx(Card, {"className": "border-border bg-card/60"}, [__jacJsx(CardContent, {"className": "flex flex-col gap-4 p-6"}, [__jacJsx("div", {"className": "flex items-center justify-between"}, [__jacJsx("span", {"className": "flex items-center gap-2 text-sm font-medium text-foreground"}, [__jacJsx(MessageSquare, {"size": 15, "className": "text-primary"}, []), "Live guestbook"]), __jacJsx(Badge, {"variant": "secondary", "className": "gap-1 font-normal"}, [__jacJsx(Database, {"size": 11}, []), String(entries.length), " ", "in graph"])]), __jacJsx("div", {"className": "flex flex-col gap-2"}, [__jacJsx(Input, {"placeholder": "Your name", "value": name, "maxLength": 40, "onChange": e => onNameChange(e.target.value)}, []), __jacJsx(Textarea, {"placeholder": "Leave a message for the graph...", "value": message, "rows": 2, "maxLength": 200, "className": "resize-none", "onChange": e => onMessageChange(e.target.value)}, []), __jacJsx(Button, {"className": "self-end rounded-full px-5 font-semibold", "disabled": (signing || !message.trim()), "onClick": () => onSign()}, [__jacJsx(Send, {"size": 15}, []), (signing ? "Signing..." : "Sign the graph")])]), __jacJsx("div", {"className": "flex max-h-72 flex-col gap-3 overflow-y-auto pr-1"}, [(() => {
    let __jac_view_kids = [];
    if ((entries.length === 0)) {
      __jac_view_kids.push(__jacJsx("div", {"className": "py-8 text-center text-sm text-muted-foreground"}, ["No signatures yet - be the first node."]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })(), entries.map(e => __jacJsx("div", {"className": "tij-fade-up flex gap-3", "key": _jac.builtin.jid(e)}, [__jacJsx(Avatar, {"className": "size-9 shrink-0"}, [__jacJsx(AvatarFallback, {"className": "bg-primary/15 text-sm text-primary"}, [(e.name ? e.name[0].toUpperCase() : "?")])]), __jacJsx("div", {"className": "min-w-0 flex-1 rounded-lg border border-border bg-background/50 px-3 py-2"}, [__jacJsx("div", {"className": "text-sm font-semibold text-foreground"}, [e.name]), __jacJsx("div", {"className": "break-words text-sm text-muted-foreground"}, [e.message])])]))])])]), __jacJsx(CodeBlock, {"code": _CODE, "filename": "guestbook.jac"}, [])]), __jacJsx("div", {"className": "mt-8 grid items-start gap-8 lg:grid-cols-2"}, [__jacJsx(Card, {"className": "border-border bg-card/60"}, [__jacJsx(CardContent, {"className": "flex flex-col gap-4 p-6"}, [__jacJsx("div", {"className": "flex items-center justify-between"}, [__jacJsx("span", {"className": "flex items-center gap-2 text-sm font-medium text-foreground"}, [__jacJsx(Footprints, {"size": 15, "className": "text-primary"}, []), "explore_graph - live walker traversal"]), __jacJsx(Button, {"size": "sm", "className": "rounded-full px-4 font-semibold", "disabled": exploring, "onClick": () => onExplore()}, [__jacJsx(Play, {"size": 14}, []), (exploring ? "Walking..." : "Spawn the walker")])]), (() => {
    let __jac_view_kids_1 = [];
    if (walkerSnap) {
      __jac_view_kids_1.push(__jacJsx("div", {"className": "flex flex-col gap-3"}, [__jacJsx("svg", {"viewBox": ("0 0 520 " + String(ly["h"])), "className": "w-full rounded-lg border border-border bg-background/50", "style": {"maxHeight": "300px"}}, [_jac.builtin.range(walkerSnap.edges.length).map(i => __jacJsx("line", {"key": ("e" + String(i)), "x1": _jac.poly.getitem(ly["pos"], walkerSnap.edges.at(i)[0])["x"], "y1": _jac.poly.getitem(ly["pos"], walkerSnap.edges.at(i)[0])["y"], "x2": _jac.poly.getitem(ly["pos"], walkerSnap.edges.at(i)[1])["x"], "y2": _jac.poly.getitem(ly["pos"], walkerSnap.edges.at(i)[1])["y"], "className": "stroke-border", "strokeWidth": 1.5}, [])), _jac.builtin.range(walkerSnap.nodes.length).map(i => __jacJsx("g", {"key": ("n" + walkerSnap.nodes.at(i).id)}, [__jacJsx("circle", {"cx": _jac.poly.getitem(ly["pos"], walkerSnap.nodes.at(i).id)["x"], "cy": _jac.poly.getitem(ly["pos"], walkerSnap.nodes.at(i).id)["y"], "r": ((walkerSnap.nodes.at(i).kind === "root") ? 13 : 10), "className": _nfill(walkerSnap.nodes.at(i).kind)}, []), __jacJsx("text", {"x": _jac.poly.getitem(ly["pos"], walkerSnap.nodes.at(i).id)["x"], "y": (_jac.poly.getitem(ly["pos"], walkerSnap.nodes.at(i).id)["y"] + 26), "textAnchor": "middle", "className": "fill-muted-foreground font-mono text-[10px]"}, [walkerSnap.nodes.at(i).label.slice(0, 14)])])), _jac.builtin.range(walkerSnap.path.length).map(i => __jacJsx("g", {"key": ("p" + String(i))}, [__jacJsx("circle", {"cx": (_jac.poly.getitem(ly["pos"], walkerSnap.path.at(i))["x"] + 14), "cy": (_jac.poly.getitem(ly["pos"], walkerSnap.path.at(i))["y"] - 14), "r": 8, "className": "fill-primary"}, []), __jacJsx("text", {"x": (_jac.poly.getitem(ly["pos"], walkerSnap.path.at(i))["x"] + 14), "y": (_jac.poly.getitem(ly["pos"], walkerSnap.path.at(i))["y"] - 11), "textAnchor": "middle", "className": "fill-primary-foreground text-[9px] font-bold"}, [String((i + 1))])]))]), __jacJsx("div", {"className": "max-h-28 overflow-y-auto rounded-lg border border-border bg-background/50 p-3 font-mono text-xs text-muted-foreground"}, [_jac.builtin.range(walkerSnap.log.length).map(i => __jacJsx("div", {"key": ("l" + String(i))}, [("> " + walkerSnap.log.at(i))]))])]));
    }
    return __jacJsx(null, {}, [__jac_view_kids_1]);
  })(), (() => {
    let __jac_view_kids_2 = [];
    if (!walkerSnap) {
      __jac_view_kids_2.push(__jacJsx("div", {"className": "flex h-48 items-center justify-center rounded-lg border border-dashed border-border text-center text-sm text-muted-foreground"}, ["Spawn the walker to traverse the real graph - root, each Day, every signature - in visit order."]));
    }
    return __jacJsx(null, {}, [__jac_view_kids_2]);
  })()])]), __jacJsx(GraphView, {"refreshKey": graphRefresh}, [])])])]);
}
/*jac:refresh-boundary*/;
export {FullstackSection};
//# sourceMappingURL=FullstackSection.js.map
