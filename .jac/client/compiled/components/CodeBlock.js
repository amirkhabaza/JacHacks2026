/* Source: /Users/amirkhabaza/JacHacks2026/components/CodeBlock.jac */
...
let _NL = String.fromCodePoint(10);
let _KEYWORDS = ["import", "from", "include", "def", "obj", "node", "edge", "walker", "enum", "has", "can", "with", "entry", "exit", "visit", "report", "spawn", "return", "if", "else", "elif", "for", "while", "in", "not", "and", "or", "is", "by", "glob", "cl", "na", "sv", "lambda", "async", "await", "yield", "True", "False", "None", "self", "here", "root", "disengage", "del", "grant", "as", "global", "let", "abs"];
function _tok_class(kind) {
  if ((kind === "kw")) {
    return "text-primary";
  }
  if ((kind === "st")) {
...
function _tokenize(src) {
  ...
  while ((i < n)) {
    ...
    if ((c === "#")) {
      let j = i;
      while (((j < n) && (src.at(j) !== _NL))) {
        j += 1;
      }
      out.push({"t": src.slice(i, j), "k": "cm"});
      i = j;
      continue;
    }
    if (((c === "\"") || (c === "'"))) {
      let triple = ((((i + 2) < n) && (src.at((i + 1)) === c)) && (src.at((i + 2)) === c));
      let j = (triple ? (i + 3) : (i + 1));
      if (triple) {
        while (((j < n) && !((((src.at(j) === c) && ((j + 2) < n)) && (src.at((j + 1)) === c)) && (src.at((j + 2)) === c)))) {
          j += 1;
        }
        j = Math.min(n, (j + 3));
      } else {
        while (((j < n) && (src.at(j) !== c))) {
          if ((src.at(j) === "\\")) {
            j += 1;
          }
          j += 1;
      ...
      out.push({"t": src.slice(i, j), "k": "st"});
      i = j;
      continue;
    }
    if (_LETTERS.includes(c)) {
      let j = i;
      while (((j < n) && (_LETTERS.includes(src.at(j)) || _DIGITS.includes(src.at(j))))) {
        j += 1;
      }
      let word = src.slice(i, j);
      let kind = "id";
      if (_KEYWORDS.includes(word)) {
        kind = "kw";
      } else if ("ABCDEFGHIJKLMNOPQRSTUVWXYZ".includes(word[0])) {
        kind = "ty";
      }
    ...
    if (_DIGITS.includes(c)) {
      let j = i;
      while (((j < n) && (_DIGITS.includes(src.at(j)) || ".xXabcdefABCDEF_".includes(src.at(j))))) {
        j += 1;
      }
      out.push({"t": src.slice(i, j), "k": "nu"});
      i = j;
      continue;
    }
    let j = i;
    while (((((((j < n) && (src.at(j) !== "#")) && (src.at(j) !== "\"")) && (src.at(j) !== "'")) && !_LETTERS.includes(src.at(j))) && !_DIGITS.includes(src.at(j)))) {
      j += 1;
    }
    if ((j === i)) {
      j = (i + 1);
    }
    out.push({"t": src.slice(i, j), "k": "pn"});
    i = j;
  }
  return out;
}
function CodeBlock(props) {
  const {code, filename: filename = "main.jac", className: className = "", dense: dense = false} = props;
  const [copied, setCopied] = useState(false);
  let tokens = _tokenize(code);
  let do_copy = () => {
    navigator.clipboard.writeText(code);
    setCopied(true);
  };
  return __jacJsx("div", {"className": ("group/code overflow-hidden rounded-xl border border-border bg-[#0c0e14] shadow-xl shadow-black/30 " + className)}, [__jacJsx("div", {"className": "flex items-center gap-2 border-b border-border/70 bg-white/[0.02] px-4 py-2.5"}, [__jacJsx("span", {"className": "size-3 rounded-full bg-[#ff5f57]"}, []), __jacJsx("span", {"className": "size-3 rounded-full bg-[#febc2e]"}, []), __jacJsx("span", {"className": "size-3 rounded-full bg-[#28c840]"}, []), __jacJsx("span", {"className": "ml-2 font-mono text-xs text-muted-foreground"}, [filename]), __jacJsx("button", {"className": "ml-auto flex items-center gap-1.5 rounded-md px-2 py-1 text-xs text-muted-foreground transition hover:bg-white/5 hover:text-foreground", "onClick": do_copy, "title": "Copy code"}, [(() => {
    let __jac_view_kids = [];
    if (copied) {
      __jac_view_kids.push(__jacJsx(Check, {"size": 13, "className": "text-emerald-400"}, []));
    } else {
      __jac_view_kids.push(__jacJsx(Copy, {"size": 13}, []));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })(), (copied ? "Copied" : "Copy")])]), __jacJsx("pre", {"className": ("overflow-x-auto p-4 text-[13px] " + (dense ? "leading-snug" : "leading-relaxed"))}, [__jacJsx("code", {"className": "font-mono"}, [_jac.builtin.enumerate(tokens).map(([idx, tok]) => __jacJsx("span", {"className": _tok_class(tok["k"]), "key": idx}, [tok["t"]]))])])]);
}
