/* Source: /Users/amirkhabaza/JacHacks2026/components/NativeSection.jac */
...
let _CODE = "# Bind a precompiled C library by its LOGICAL name - no\n# headers, no `cc`, no per-OS path. The native backend picks\n# libraylib.so / .dylib / raylib.dll from the target triple.\nimport from raylib {\n    def InitWindow(width: i32, height: i32, title: str) -> None;\n    def rlVertex3f(x: f32, y: f32, z: f32) -> None;\n    def rlColor4ub(r: u8, g: u8, b: u8, a: u8) -> None;\n}\n\n# `jac nacompile shooter.jac` emits a standalone ELF/Mach-O.\n# The linker is PURE PYTHON - there is no cc/ld invocation.";
function NativeSection() {
  const [jacAvg, setJacAvg] = useState(461.7);
  const [jacMax, setJacMax] = useState(719.9);
  const [jacFrames, setJacFrames] = useState(3694);
  const [zigAvg, setZigAvg] = useState(470.5);
  const [zigMax, setZigMax] = useState(719.1);
  const [zigFrames, setZigFrames] = useState(3764);
  const [capturedAt, setCapturedAt] = useState("");
  const [jacShotOk, setJacShotOk] = useState(true);
  const [zigShotOk, setZigShotOk] = useState(true);
  useEffect(() => {
    loadBench();
  }, []);
  async function loadBench() {
    try {
      let resp = await fetch("/static/captures/benchmark.json");
      if (resp.ok) {
        let data = await resp.json();
        if ((data["jac"] && data["jac"]["avg"])) {
          setJacAvg(data["jac"]["avg"]);
          setJacMax(data["jac"]["max"]);
          setJacFrames(data["jac"]["frames"]);
        }
        if ((data["zig"] && data["zig"]["avg"])) {
          setZigAvg(data["zig"]["avg"]);
          setZigMax(data["zig"]["max"]);
          setZigFrames(data["zig"]["frames"]);
        }
        setCapturedAt((data["captured_at"] ? data["captured_at"] : ""));
      }
    } catch (__jac_e) {
  ...
  let scale = (Math.max(jacAvg, zigAvg) * 1.08);
  return __jacJsx(SectionShell, {"id": "native", "eyebrow": "Native", "title": "Compiles to machine code, no C toolchain", "subtitle": "A first-person shooter written in Jac, linked against precompiled raylib and built to a standalone binary - benchmarked head-to-head with a hand-written Zig twin doing identical work."}, [__jacJsx("div", {"className": "grid items-start gap-8 lg:grid-cols-2"}, [__jacJsx(CodeBlock, {"code": _CODE, "filename": "shooter.jac"}, []), __jacJsx(Card, {"className": "border-border bg-card/60"}, [__jacJsx(CardContent, {"className": "flex flex-col gap-5 p-6"}, [__jacJsx("div", {"className": "flex items-center justify-between"}, [__jacJsx("span", {"className": "flex items-center gap-2 text-sm font-medium text-foreground"}, [__jacJsx(Zap, {"size": 15, "className": "text-primary"}, []), "raylib cube shooter - uncapped FPS"]), __jacJsx(Badge, {"variant": "secondary", "className": "font-normal"}, [(capturedAt ? capturedAt : "representative")])]), __jacJsx(StatBar, {"label": "Jac (nacompile)", "value": jacAvg, "max_value": scale, "display": (String(_jac.builtin.round(jacAvg, 1)) + " avg fps"), "barClass": "bg-primary", "sub": (((("peak " + String(_jac.builtin.round(jacMax, 1))) + " \u00b7 ") + String(jacFrames)) + " frames")}, []), __jacJsx(StatBar, {"label": "Zig (baseline)", "value": zigAvg, "max_value": scale, "display": (String(_jac.builtin.round(zigAvg, 1)) + " avg fps"), "barClass": "bg-sky-400", "sub": (((("peak " + String(_jac.builtin.round(zigMax, 1))) + " \u00b7 ") + String(zigFrames)) + " frames")}, []), __jacJsx("div", {"className": "flex items-center gap-2 rounded-lg border border-border bg-background/50 p-3 text-sm text-muted-foreground"}, [__jacJsx(Cpu, {"size": 15, "className": "shrink-0 text-primary"}, []), "Same precompiled raylib, identical per-frame work - Jac tracks the conventional toolchain within the margin."])])])]), __jacJsx("div", {"className": "mt-8 grid gap-4 md:grid-cols-2"}, [__jacJsx("div", {"className": "overflow-hidden rounded-xl border border-border bg-[#0b0e16]"}, [__jacJsx("div", {"className": "flex items-center gap-2 border-b border-border/60 px-4 py-2 text-xs text-muted-foreground"}, [__jacJsx(Camera, {"size": 13}, []), "Jac build (jac nacompile) - captured by the engine itself"]), (() => {
    let __jac_view_kids = [];
    if (jacShotOk) {
      __jac_view_kids.push(__jacJsx("img", {"src": "/static/captures/shooter_jac.png", "alt": "Jac native raylib cube shooter", "className": "aspect-[16/10] w-full object-cover", "onError": () => {
        setJacShotOk(false);
      }}, []));
    } else {
      __jac_view_kids.push(__jacJsx("div", {"className": "flex aspect-[16/10] w-full flex-col items-center justify-center gap-2 text-muted-foreground"}, [__jacJsx(Camera, {"size": 24, "className": "opacity-40"}, []), __jacJsx("span", {"className": "text-xs"}, ["run capture.jac to add a screenshot"])]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })()]), __jacJsx("div", {"className": "overflow-hidden rounded-xl border border-border bg-[#0b0e16]"}, [__jacJsx("div", {"className": "flex items-center gap-2 border-b border-border/60 px-4 py-2 text-xs text-muted-foreground"}, [__jacJsx(Camera, {"size": 13}, []), "Zig twin (zig build-exe) - captured by the engine itself"]), (() => {
    let __jac_view_kids_1 = [];
    if (zigShotOk) {
      __jac_view_kids_1.push(__jacJsx("img", {"src": "/static/captures/shooter_zig.png", "alt": "Zig twin raylib cube shooter", "className": "aspect-[16/10] w-full object-cover", "onError": () => {
        setZigShotOk(false);
      }}, []));
    } else {
      __jac_view_kids_1.push(__jacJsx("div", {"className": "flex aspect-[16/10] w-full flex-col items-center justify-center gap-2 text-muted-foreground"}, [__jacJsx(Camera, {"size": 24, "className": "opacity-40"}, []), __jacJsx("span", {"className": "text-xs"}, ["run capture.jac to add a screenshot"])]));
    }
    return __jacJsx(null, {}, [__jac_view_kids_1]);
  })()])])]);
}
/*jac:refresh-boundary*/;
export {NativeSection};
//# sourceMappingURL=NativeSection.js.map
